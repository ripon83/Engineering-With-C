/*
 * Program: Dynamic Array Allocation 9
...
#include <stdio.h>
#include <stdlib.h>
int *reduction(int *a, int n, int m)
{
    
    int *b=(int *)malloc(sizeof(int)*(n-m));  // Allocate memory dynamically
    for(int i=0; i<n-m; i++)  // Iterate through array elements
    {
...
int main()  // Main function: execution starts here
{
    ...
    int m=5;
    arr=reduction(arr, n, m);
    for(i=n; i<n-m; i++)  // Iterate through array elements
        arr[i]=i+1;  // Store a value in an array element
    printf("\nAfter the reduction of the array size:\n");  // Display output

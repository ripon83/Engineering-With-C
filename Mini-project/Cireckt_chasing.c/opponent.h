...
// void setupMatchAuto(match *m2, match *m1) 
// {
//     int ch;
...
void currentSummaryAuto(match *m, int ball) 
{
    printf("\n=========================Current summary========================");
...
void playBallAuto(struct Match *m, int ball) 
{
    printf("\nBall %d ", ball);
    int input = getRun();
    if (input == 7) 
...
void playOverAuto(struct Match *m, int over, int *flag) 
{
    char ch[100];
    printf("\n===== OVER %d =====\n", over);
    for (int b = 1; b <= 6; b++) 

...
int userKickDirection()
{
    ...
    printf("Kick the ball to goal. Select a side: ");
    scanf("%d",&ch);
    return ch;
}
...
int userSaveKick()
{
    ...
    printf("\n1. Left    2. Center    3. Right\n");
    printf("Save the ball kicked by computer: ");
    scanf("%d",&ch);
    return ch;
}
...
void play(score *s)
{
    ...
    if(s->user > s->comp)
    {
        printf("\n****************************************************************\n");
        printf("\n\t\t\t\t🏆\n");
        printf("\t\tCongratulations! You have won! ✌️\n");
    ...
    {
        printf("\t\t You have lost the match by %d goal. 💀\n",s->comp - s->user);
    }
}
